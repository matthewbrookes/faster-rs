        ...
        if let Ok(recover_store) = FasterKv::new(TABLE_SIZE, LOG_SIZE, STORAGE_DIR.to_string()) {
            match recover_store.recover(token.clone(), token.clone()) {
                Some(rec) => {
                    ...
                    let persisted_count = recover_store.continue_session(rec.session_ids.first().cloned().unwrap());
                    ...
                    for i in 0..NUM_OPS {
                        let idx = i as u64;
                        let (status, recv) : (u8, Receiver<u64>) = recover_store.read(idx % NUM_UNIQUE_KEYS, idx);
